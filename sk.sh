...
sudo iptables -t nat -A PREROUTING -i ens4 -p tcp -m tcp --dport 80 -j DNAT --to-destination 172.17.0.3
sudo iptables -t nat -A PREROUTING -i ens4 -p tcp -m tcp --dport 443 -j DNAT --to-destination 172.17.0.3
sudo iptables -t nat -A PREROUTING -i ens4 -p tcp -m tcp --dport 222 -j DNAT --to-destination 172.17.0.3:22
...
sudo -u root -s sh -c 'echo "set_var EASYRSA_REQ_ORG        "EUROSKILLS"" >> /etc/easy-rsa/vars'
sudo -u root -s sh -c 'echo "set_var EASYRSA_REQ_EMAIL      "me@skills.pt"" >> /etc/easy-rsa/vars'
sudo -u root -s sh -c 'echo "set_var EASYRSA_REQ_OU         "EUROSKILLS"" >> /etc/easy-rsa/vars'
